  ...
  <div>
    ...
    <div id="table">
      ...
      >
        ...
        <el-table-column label="涉及疾病" prop="disease"> </el-table-column>
...
import { disOptions } from "@/components/mixins/constData.js";
...
export default {
  ...
  methods: {
      ...
      getRequest("/DataTable/upall", {}).then((res) => {
        ...
        for (let i = 0; i < tempList.length; i++) {
          const obj = {
            ...
            tablePeople: tempList[i].creator,
            tableOrigin: tempList[i].tableDesc,
            tableSize: (tempList[i].samplesize).toString(10) + '×' + (tempList[i].featurenumber).toString(10),
            tableDate: tempList[i].createTime,
            tableStatus: tempList[i].tableStatus,
            disease: tempList[i].disease,
          };
          // console.log("obj = " + obj);
          console.log("tableName = " + tempList[i].table_name);
          this.tableData.push(obj);
        }
      });
      // let te = this.tableData;
      // console.log(te)
    ...
    handleDelete(tableName) {
      ...
      postRequest(`DataTable/delete/${tableName}`).then((res) => {
    ...
    upRequest(data) {
      ...
      payload.append("createName", sessionStorage.getItem("username"),);
      console.log("你好");
      sessionStorage.setItem("file", data.file);
      sessionStorage.setItem("newName", this.dialogForm.tableName);
      sessionStorage.setItem("disease", this.dialogForm.dataDisease);
      sessionStorage.setItem("createName", sessionStorage.getItem("username"));
      const options = {
        method: "post",
        data: payload,
    ...
    compelete() {
      ...
      postRequest("/DataTable/insertTableManager", {
      ...
      const payload = new FormData();
      console.log("file:", sessionStorage.getItem("file"))
      payload.append("file", sessionStorage.getItem("file"));
      payload.append("newName", sessionStorage.getItem("newName"));
      payload.append("disease", sessionStorage.getItem("disease"));
      payload.append("createName", sessionStorage.getItem("createName"));
      payload.append("uid", sessionStorage.getItem("userid"));
      console.log("createName", sessionStorage.getItem("createName"));
      console.log("uid", sessionStorage.getItem("userid"));
      
      const options = {
        method: "post",
        data:payload,
        url: "/DataTable/uploadTable",
        // headers: {
        //   "Content-Type": "multipart/form-data",
        // },
      };
